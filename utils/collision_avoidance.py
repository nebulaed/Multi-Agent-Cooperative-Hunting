...
from utils.math_func import correct, peri_arctan, arcsin, norm, inc_angle, intervals_merge, expand_angle
from utils.params import WOLF_NUM, PI

def robot_avoid_obs(t: int, mark: int, vel_wolf_desired: float, theta_wolf_desired: float, my_t: int, wolves: List[Robot], mob_obss: List[MobObs], sta_obss: List[StaObs], irr_obss: List[IrregularObs], m_irr_obss: List[MobIrregularObs], border: Border, D_DANGER: float, D_DANGER_W: float):
    """
    围捕机器人的避障算法
    ...
    if len(dict_d_obs) != 0:
        for item in dict_d_obs:
            ...
            if index[0] == 0:
                ...
                if not flag:
                    vel_wolf_desired = 0.1 * wolves[mark].vel_max
            # 若是固定障碍物
            elif index[0] == 1:
                i_s = index[1]
                dist_s = item[1]
